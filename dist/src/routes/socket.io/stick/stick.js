...
const stick__event_res_1 = __importDefault(require("./stick..event.res"));
const CONSTANT_1 = __importDefault(require("./CONSTANT"));
const collisionCard_res_1 = __importDefault(require("./collisionCard.res"));
class Stick {
    constructor(socket, io) {
        ...
        this.vx = 4;
        this.eventsState = Object.assign({}, CONSTANT_1.default);
    }
    // #region get/set
    getId() {
        return this._id;
    ...
    stand({ event }) {
        ...
        const mainShape = createShapeStick(location.x, location.y, gameConfig_json_1.default.stick.w * gameConfig_json_1.default.stick.scale, gameConfig_json_1.default.stick.h * gameConfig_json_1.default.stick.scale);
        const shapesStair = this.socket.handshake.match.stairs.map((stair) => createShapeStair(stair.x, stair.y, stair.width, stair.height));
        const nearest = findNearest(mainShape, shapesStair);
        const standingPlace = Math.min(gameConfig_json_1.default.stairGame.height, nearest.bottomNearest[0].y);
        if (standingPlace > location.y) {
            ...
            this.socket.handshake.match.endEventTime = Math.abs(new Date().getTime() + fall.time * 1000);
        ...
        this.io
            ...
            .emit(this.EVENT, new stick__event_res_1.default(this._id, event, location.x, location.y));
        return;
    }
    left({ event }) {
        this.setId(this.getId());
        ...
        const location = this.socket.handshake.match.player.stairGame;
        const mainShape = createShapeStick(location.x, location.y, gameConfig_json_1.default.stick.w * gameConfig_json_1.default.stick.scale, gameConfig_json_1.default.stick.h * gameConfig_json_1.default.stick.scale);
        const stairShapes = this.socket.handshake.match.stairs.map((stair) => createShapeStair(stair.x, stair.y, stair.width, stair.height));
        const cardShapes = this.socket.handshake.match.cards.map((card) => createShapeCard(JSON.parse(card.x), JSON.parse(card.y), gameConfig_json_1.default.card.width, gameConfig_json_1.default.card.height));
        ...
        if (Number.isFinite(distanceWithCard) &&
            ...
            for (const card of this.socket.handshake.match.cards) {
                if (JSON.parse(card.x) === nearestCards.leftNearest[3].x && !card.owner) {
                    ...
                    this.io
                        .to(this.socket.handshake.idRoom)
                        .emit(this.EVENT_COLLISION_CARD, new collisionCard_res_1.default(0, this._id, card._id, card.data));
                }
            }
        }
        ...
        if (standingPlace > location.y) {
            ...
            //     location.y,
            // )
            if (Number.isFinite(cardBottomCollision) &&
                numOfCardsPickUp < 5 &&
                cardBottomCollision >= location.y) {
                const distanceWithBottomCard = cardBottomCollision - location.y;
                const cardPickedUp = this.socket.handshake.match.cards.find((card) => JSON.parse(card.x) === nearestCards.bottomNearest[3].x &&
                ...
                if (cardPickedUp && !cardPickedUp.owner) {
                    ...
                    //     location,
                    // )
                    cardPickedUp.timeOut = setTimeout(timeOutUpdateCard.bind(this), timeMilliSeconds, [this._id, cardPickedUp]);
                }
            }
            ...
            // console.log(fall, distanceToStandingPlace)
            const timeMilliSeconds = fall.time * 1000;
            this.socket.handshake.match.endEventTime = Math.abs(new Date().getTime() + timeMilliSeconds);
        ...
        this.io
            .to(this.socket.handshake.idRoom)
            .emit(this.EVENT, new stick__event_res_1.default(this._id, event, location.x, location.y, specialEvent));
    }
    right({ event }) {
        this.setId(this.getId());
        const location = this.socket.handshake.match.player.stairGame;
    ...
    jumpLeft({ event }) {
        if (this.socket.handshake.match.eventStateSpecial === this.eventsState.freeFall)
            return;
        this.setId(this.getId());
        const location = this.socket.handshake.match.player.stairGame;
        location.vx = -4;
...
function timeOutUpdateCard([_id, card]) {
    ...
    if (numOfCardsPickUp < 5 && cardFound) {
        const location = this.socket.handshake.match.player.stairGame;
        const shapeStick = createShapeStick(location.x, location.y, gameConfig_json_1.default.stick.w * gameConfig_json_1.default.stick.scale, gameConfig_json_1.default.stick.h * gameConfig_json_1.default.stick.scale);
        const shapeCard = createShapeCard(JSON.parse(cardFound.x), JSON.parse(cardFound.y), gameConfig_json_1.default.card.width, gameConfig_json_1.default.card.height);
        const checkCollisionX = (shapeStick[0].x >= shapeCard[0].x && shapeStick[0].x <= shapeCard[1].x) ||
            (shapeStick[1].x >= shapeCard[0].x && shapeStick[1].x <= shapeCard[1].x) ||
            (shapeStick[0].x <= shapeCard[0].x && shapeStick[1].x >= shapeCard[1].x);
        const checkCollisionY = (shapeStick[0].y >= shapeCard[0].y && shapeStick[0].y <= shapeCard[3].y) ||
        ...
        if (checkCollisionX && checkCollisionY && !cardFound.owner) {
            ...
            this.io
                .to(this.socket.handshake.idRoom)
                .emit(this.EVENT_COLLISION_CARD, new collisionCard_res_1.default(0, _id, cardFound._id, cardFound.data));
        }
    }
}
...
function findNearest(mainShape, otherShapes) {
    ...
    for (const shape of otherShapes) {
        // #region main check
        const checkXTop = (mainShape[0].x >= shape[0].x && mainShape[0].x <= shape[1].x) ||
            (mainShape[1].x >= shape[0].x && mainShape[1].x <= shape[1].x) ||
            (mainShape[0].x >= shape[0].x && mainShape[1].x <= shape[1].x);
        const checkXBottom = (mainShape[0].x >= shape[0].x && mainShape[0].x <= shape[1].x) ||
            (mainShape[1].x >= shape[0].x && mainShape[1].x <= shape[1].x) ||
            (mainShape[0].x >= shape[0].x && mainShape[1].x <= shape[1].x);
        const checkYRight = (mainShape[0].y >= shape[0].y && mainShape[0].y <= shape[3].y) ||
            (mainShape[3].y >= shape[0].y && mainShape[3].y <= shape[3].y) ||
            (mainShape[0].y <= shape[0].y && mainShape[3].y >= shape[3].y);
        const checkYLeft = (mainShape[0].y >= shape[0].y && mainShape[0].y <= shape[3].y) ||
            (mainShape[3].y >= shape[0].y && mainShape[3].y <= shape[3].y) ||
            (mainShape[0].y <= shape[0].y && mainShape[3].y >= shape[3].y);
