...
import { stairSchema } from '../app/schemas'
...
export function startNewTurn(_this: any, match: any) {
    console.log('\n-----------------------')
    console.log('End this turn, start new turn!, ')
    console.log('-----------------------')
    // #region compute before end turn
    // #region status reborn
    const updateStatuses: Array<any> = []
    const basePlayer = match.players.forEach((p: any) => {
    ...
    if ((teamA.onRoom.length === 0 || teamA.canPlay.length === 0) && teamB.onRoom.length !== 0) {
        teamA.onRoom.forEach((p) => {
            ...
            UserModel.updateOne(
                ...
                .then((result: any) => console.log('Update success'))
