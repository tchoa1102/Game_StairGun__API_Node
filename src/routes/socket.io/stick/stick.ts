...
import StickEventRes from './stick..event.res'
import CONSTANT_STICK from './CONSTANT'
import CollisionCardRes from './collisionCard.res'
...
class Stick {
    ...
    public io: any
    public EVENT: string
    public EVENT_COLLISION_CARD: string
    public _id: any
    public vx: number
    ...
    stand({ event }: any) {
        ...
        const shapesStair = this.socket.handshake.match.stairs.map((stair: any) =>
        ...
        if (standingPlace > location.y) {
            ...
            this.socket.handshake.match.endEventTime = Math.abs(
                new Date().getTime() + fall.time * 1000,
            )
    ...
    left({ event }: any) {
        ...
        const stairShapes = this.socket.handshake.match.stairs.map((stair: any) =>
            createShapeStair(stair.x, stair.y, stair.width, stair.height),
        )
        const cardShapes = this.socket.handshake.match.cards.map((card: any) =>
            createShapeCard(
                JSON.parse(card.x),
                JSON.parse(card.y),
                config.card.width,
        ...
        const numOfCardsPickUp = this.socket.handshake.match.cards.reduce(
        ...
        if (standingPlace > location.y) {
            ...
            const timeMilliSeconds = fall.time * 1000
            this.socket.handshake.match.endEventTime = Math.abs(
                new Date().getTime() + timeMilliSeconds,
            )
...
function timeOutUpdateCard(this: any, [_id, card]: any) {
    // console.log('Arguments: ', card._id)
    let cardFound: any = null
    const numOfCardsPickUp = this.socket.handshake.match.cards.reduce(
        (total: number, curCard: any) => {
...
function freeFall({ x, y }: any, kDistance: number) {
...
function findNearest(mainShape: any, otherShapes: any) {
    const baseMinShape = [
    ...
    function minFor(curShape: any, newShape: any, s1Point: any, s2Point: any, axis: any) {
        return curShape[s1Point][axis] > newShape[s2Point][axis] ? [...newShape] : curShape
    }

    function maxFor(curShape: any, newShape: any, s1Point: any, s2Point: any, axis: any) {
...
function createShapeStick(x3: number, y3: number, w: number, h: number) {
    return [
        { x: x3 - w, y: y3 - h },
        { x: x3, y: y3 - h },
        { x: x3, y: y3 },
...
function createShapeStair(x1: number, y1: number, w: number, h: number) {
    return [
        { x: x1, y: y1 },
        { x: x1 + w, y: y1 },
        { x: x1 + w, y: y1 + h },
...
function createShapeCard(x4: number, y4: number, w: number, h: number) {
    return [
        { x: x4, y: y4 - h },
        { x: x4 + w, y: y4 - h },
        { x: x4 + w, y: y4 },
